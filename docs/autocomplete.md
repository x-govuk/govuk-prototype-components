...
| items              | array   | **Required.** Array of option items for the autocomplete. See [options for items](#options-for-items-array-objects).                                                                                                                                                                                                         |
...
| minLength          | integer | Minimum number of characters that should be entered before the autocomplete will attempt to suggest options. When the query length is under this, the aria status region will also provide helpful text to the user informing them they should type in more (default is `0`)                                                 |
...
| label              | object  | Label text or HTML by specifying value for either text or html keys. See [options for label](#options-for-label-object).                                                                                                                                                                                                     |
| hint               | object  | Options for the hint component. See [options for hint](#options-for-hint-object).                                                                                                                                                                                                                                            |
| errorMessage       | object  | Options for the error message component. The error message component will not display if you use a falsy value for `errorMessage`, for example `false` or `null`. See [options for error message](https://design-system.service.gov.uk/components/error-message/#options-error-message-example) in the GOV.UK Design System. |
| formGroup          | object  | Options for the form-group wrapper. See [options for formGroup](#options-for-form-group-object).                                                                                                                                                                                                                             |
